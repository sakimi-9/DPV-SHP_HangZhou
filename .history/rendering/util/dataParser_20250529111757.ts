...
export function parseHouseData(rawData: any[]): ParsedHouse[] {
    ...
    rawData.forEach(item => {
        // Use the actual keys from the JSON data
        const parsedItem: ParsedHouse = {
            ...
            所在区域: null,
            单价: null,
            房源标题: item['house_list'] || '', // Map 'house_list' to '房源标题'
            关注人数: null,
            发布时间: item['followers_and_post_time'] ? item['followers_and_post_time'].split(' / ')[1]?.trim() || null : null,
            标签: item['__EMPTY'] ? item['__EMPTY'].split(' ').filter((tag: string) => tag.trim() !== '') : [], // Map '__EMPTY' to '标签' (assuming tags are here)
        };

        // 解析 "房子信息" (对应 JSON 中的 'house_info')
        const houseInfoParts = item['house_info'] ? item['house_info'].split(' | ').map((part: string) => part.trim()) : [];
        ...
        // 解析 "所在区域" (对应 JSON 中的 'district')
        const areaParts = item['district'] ? item['district'].split('  ').map((part: string) => part.trim()) : [];
        if (areaParts.length > 0) {
            parsedItem.小区名称 = areaParts[0];
            if (areaParts.length > 1) {
                parsedItem.所在区域 = areaParts[1]; // Assign the second part as the area
            }
        }

        // 解析 "单价" (对应 JSON 中的 'unit_price')
        const priceMatch = item['unit_price'] ? item['unit_price'].replace(',', '').match(/([\d\.]+)元\/平/) : null;
        if (priceMatch) {
            parsedItem.单价 = parseFloat(priceMatch[1]);
        }

        // 解析 "关注人数和发布时间" (对应 JSON 中的 'followers_and_post_time')
        const followTimeParts = item['followers_and_post_time'] ? item['followers_and_post_time'].split(' / ').map((part: string) => part.trim()) : [];
