...
export function getAreaPriceBoxPlotData(data: ParsedHouse[]): { area: string; prices: number[] }[] {
    console.log("getAreaPriceBoxPlotData received", data ? data.length : 0, "items.");
...
export function getTop10FollowedHousesData(data: ParsedHouse[]): { title: string; followers: number }[] {
    console.log("getTop10FollowedHousesData received", data ? data.length : 0, "items.");
...
export function getRenovationPriceScatterData(data: ParsedHouse[]): { renovation: string; price: number }[] {
    console.log("getRenovationPriceScatterData received", data ? data.length : 0, "items.");
    return data
        .filter(house => {
...
export function getAreaHouseCountPieData(data: ParsedHouse[]): { area: string; count: number }[] {
    console.log("getAreaHouseCountPieData received", data ? data.length : 0, "items.");
...
export function getPublishTimeFollowersTrendData(data: ParsedHouse[]): { time: string; totalFollowers: number }[] {
    ...
    console.log("getPublishTimeFollowersTrendData received", data ? data.length : 0, "items.");
    data.forEach(house => {
    ...
    console.log("getPublishTimeFollowersTrendData processed into", Object.keys(timeFollowers).length, "data points.");
