...
const renderSelectedChart = (data: ParsedHouse[] | null) => {
    ...
    const chartIndex = selectedChartIndex.value;
    if (chartIndex === null || chartIndex < 0 || chartIndex >= charts.length) return; // Ensure index is valid

    const chartInfo = charts[chartIndex]; // Get the chart info directly from the array
