<template>
    <div class="charts-container">
        ...
        <div class="chart-display-area">
            <div v-if="selectedChartIndex !== null && currentChart">
...
// Use an array of refs to dynamically get the ref for the selected chart
...
const currentChart = computed(() => {
    ...
    // Ensure the ref for the current chart exists
    const chartInfo = charts[selectedChartIndex.value];
    if (chartRefs.value[selectedChartIndex.value]) {
        return {
            ...chartInfo,
            ref: chartRefs.value[selectedChartIndex.value] // Use the actual ref from the array
        };
    } else {
        return null; // Return null if the ref is not ready
    }
});
...
const renderSelectedChart = (data: ParsedHouse[] | null) => {
    ...
    if (chartRefs.value.length > 0 && selectedChartIndex.value !== null && chartRefs.value[selectedChartIndex.value]) {
        disposeChart(chartRefs.value[selectedChartIndex.value] as HTMLElement);
    }

    if (selectedChartIndex.value === null || !data || data.length === 0 || !currentChart.value) {
        console.warn("No chart selected, no data, or chart ref not ready.");
        // Display placeholder or empty state
        const displayArea = document.querySelector('.chart-display-area');
        if (displayArea) {
            const chartContainer = displayArea.querySelector('[ref]'); // Find the chart container div
            if (chartContainer) {
                disposeChart(chartContainer as HTMLElement);
            }
            // You could also update the placeholder message here if needed
        }
        return;
    }

    const chartElement = currentChart.value.ref; // Use the ref from the computed property
...
watch([() => props.houseData, selectedChartIndex], async ([newData, newIndex], [oldData, oldIndex]) => {
...
// Assign refs dynamically using a function in the template
const setChartRef = (el: any, index: number) => {
    if (el) {
        chartRefs.value[index] = el;
    } else if (chartRefs.value[index]) { // If element is null, clear the ref
        chartRefs.value[index] = null;
    }
};

// Clear all chart instances when data changes or component is unmounted
// Not strictly necessary with the dispose logic in renderSelectedChart, but good practice
watch(() => props.houseData, (newData, oldData) => {
    if (newData !== oldData) {
        ...
        chartRefs.value = []; // Clear refs array
        selectedChartIndex.value = null; // Reset selection
    }
});

// Dispose charts when component is unmounted
onMounted(() => {
    // Initial render of placeholder if no data/selection
