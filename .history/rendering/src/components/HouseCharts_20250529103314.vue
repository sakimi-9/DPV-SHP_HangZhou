...
const renderSelectedChart = (data: ParsedHouse[] | null) => {
    ...
    if (selectedChartIndex.value === null || !data || data.length === 0 || !currentChart.value) {
        ...
        if (displayArea && selectedChartIndex.value === null) { // Only show placeholder if no chart is selected
            displayArea.innerHTML = '<div class="placeholder-message"><p>请点击上方的卡片查看图表</p></div>';
        }
        return;
    }
...
watch([() => props.houseData, selectedChartIndex], async ([newData, newIndex]) => {
    // Wait for the DOM to update after selectedChartIndex changes and the ref is available
    await nextTick();
...
.charts-grid {
    ...
    overflow-x: auto;
    /* Add scroll if cards exceed max width */
}
...
.chart-card {
    ...
    flex: 1 0 auto;
    /* Allow cards to grow/shrink but maintain content width initially */
    min-width: 180px;
    /* Give cards a minimum width to not become too small */
    max-width: 200px;
    /* Give cards a maximum width */
}
