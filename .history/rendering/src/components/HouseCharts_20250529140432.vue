    ...
    <div class="charts-container">
        ...
        <div class="chart-display-area">
            <div v-if="selectedChartIndex !== null && currentChart" class="selected-chart-content">
                ...
                <div ref="selectedChartDisplayRef" style="width: 100%; height: 70vh;"></div>
            </div>
...
watch([() => props.houseData, selectedChartIndex], async ([newData, newIndex]) => {
    ...
    if (newIndex !== null && newData && newData.length > 0) {
        console.log("Data and Index valid, waiting for nextTick...");
        await nextTick(); // Wait for DOM updates

        const chartElement = selectedChartDisplayRef.value;
        if (chartElement) {
            console.log("Ref element available, attempting to render chart.", "Index:", newIndex);
            // Get the chart info directly from the charts array based on the index
            const chartInfo = charts[newIndex];
            if (chartInfo) {
                renderSelectedChart(chartInfo, newData);
            } else {
                console.error("Chart info not found for index:", newIndex);
            }
        } else {
            console.warn("Ref element not available after nextTick.");
        }
    } else {
        // If conditions are not met, dispose the current chart if it exists
        if (selectedChartDisplayRef.value) {
            console.log("Conditions not met, disposing chart.");
            disposeChart(selectedChartDisplayRef.value);
            selectedChartDisplayRef.value.innerHTML = '';
        }
    }
...
function renderAreaPriceBoxPlot(chartElement: HTMLElement, data: ParsedHouse[]) {
    ...
    console.log("Setting option for:", "各区域房源单价", option);
...
function renderRenovationPriceScatter(chartElement: HTMLElement, data: ParsedHouse[]) {
    ...
    const myChart = echarts.init(chartElement);
    console.log("ECharts instance initialized for:", "不同装修类型房源单价");
    const option = {
        title: {
            text: '不同装修类型房源单价-散点图',
            left: 'center'
...
const charts = [
    ...
    { title: '不同装修类型房源单价', render: renderRenovationPriceScatter },
    { title: '各区域房源数量占比', render: renderAreaCountPie },
    { title: '发布时间与关注人数变化趋势', render: renderPublishTimeFollowersLine },
];
...
.chart-display-area {
    ...
    align-items: center;
    min-height: 55vh;
    /* Ensure a minimum height for the box, adjusted for larger chart */
...
/* --- ECharts 图表实际渲染区域样式 (div[ref=selectedChartDisplayRef]) --- */
/* 调整这里的 width 和 height 可以直接改变图表的大小 */
