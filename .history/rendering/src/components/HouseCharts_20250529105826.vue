...
const renderSelectedChart = (data: ParsedHouse[] | null) => {
    ...
    if (!chartElement) {
        ...
        console.log("Render conditions not met.", "Index:", selectedChartIndex.value, "Data present:", !!data, "Data length:", data ? data.length : 0, "Element available:", !!chartElement);

        return; // Exit if the element is null
    }

    const chartInfo = currentChart.value; // Get the chart info
...
watch(selectedChartDisplayRef, async (newChartElement) => {
    await nextTick();
    console.log("selectedChartDisplayRef changed. New element:", !!newChartElement);
    console.log("Checking render conditions in ref watcher:", "Element available:", !!newChartElement, "Index:", selectedChartIndex.value, "Data valid:", !!props.houseData && props.houseData.length > 0, "currentChart valid:", !!currentChart.value);
    if (newChartElement && selectedChartIndex.value !== null && props.houseData && props.houseData.length > 0 && currentChart.value) {
        ...
        renderSelectedChart(props.houseData); // Call render with current data
    }
});
