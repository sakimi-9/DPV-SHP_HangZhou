...
watch(selectedChartDisplayRef, async (newChartElement) => {
    ...
    if (newChartElement && selectedChartIndex.value !== null && props.houseData && props.houseData.length > 0 && currentChart.value) {
        ...
        const chartIndex = selectedChartIndex.value; // Get the current index
        if (chartIndex !== null && chartIndex >= 0 && chartIndex < charts.length) {
            renderSelectedChart(charts[chartIndex], props.houseData); // Call render with the specific chart info
        } else {
            console.error("Invalid selectedChartIndex in ref watcher:", chartIndex);
        }
    }
});
