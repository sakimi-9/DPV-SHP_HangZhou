...
function renderRenovationPriceScatter(chartElement: HTMLElement, data: ParsedHouse[]) {
    ...
    const renovationTypes = Array.from(new Set(chartData.map((item: { renovation: string, price: number }) => item.renovation))) as string[];
...
function renderAreaCountPie(chartElement: HTMLElement, data: ParsedHouse[]) {
    ...
    const option = {
        ...
        legend: {
            orient: 'vertical',
            left: 'left',
            data: chartData.map((item: { area: string, count: number }) => item.area)
        },
        series: [
            {
                name: '房源数量',
